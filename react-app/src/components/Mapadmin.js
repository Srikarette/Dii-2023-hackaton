...
const Mapadmin = () => {
  ...
  const handlePostMarker = async (category, latitude, longitude) => {
    try {
      const response = await axios.post("https://noti-service.azurewebsites.net/notifications", {
        category: category,
        latitude: latitude,
        longitude: longitude,
      });
  ...
  const handleDeleteMarker = (notificationId) => {
    ...
    axios
      .delete(`https://noti-service.azurewebsites.net/notifications/${notificationId}`)
      .then((response) => {
        if (response.status === 200) {
  ...
  const handleSubmitEdit = (e) => {
    ...
    axios
      .patch(`https://noti-service.azurewebsites.net/notifications/${id}`, formData, {
        headers: {
